...
describe("Alert.vue", () => {
  ...
  it("should support centering text", () => {
    ...
    //class
    const rootNode = wrapper.find(".toy-alert");
  ...
  it("should not render close icon when closable is false", () => {
    ...
    expect(wrapper.find(".toy-alert__close").exists()).toBe(false);
  });
  ...
  it("should toggle visibility when open and close methods are called", async () => {
    ...
    expect(wrapper.find(".toy-alert").attributes().style).toBe("display: none;");
    await wrapper.vm.open();
    expect(wrapper.find(".toy-alert").attributes().style).toBe("");
  });
});
